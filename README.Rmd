...
mutate(d_stack, aic = map_dbl(output, possibly(MuMIn::AICc, NA))) %>%
  filter(., !is.na(aic)) %>%
  group_by(curve_id) %>%
  top_n(1, desc(aic)) %>%
  group_by(model) %>%
...
I will demonstrate how it works by averaging across the 3 biological replicates for a given curve, calculating the standard deviation, $\sigma$ for each mean rate value and fitting the Sharpe-Schoolfield model for high temperature inactivation to both sets of curves.

```{r example_model_weights}
