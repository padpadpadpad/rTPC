...
We can bootstrap our models to (a) visualise uncertainty of our model and its predictions and (b) allow us to quantify the uncertainty of the derived parameters in __est_params()__. This example uses non-parametric bootstrapping, where we sample from the residuals of the original model fit. This is an identical approach to that implemented in __nlstools::nlsBoot()__, but we retain each model fit in order to calculate confidence intervals for predictions and derived parameters.
