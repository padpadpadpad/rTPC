...
**Figure 2. Potential applications for fitting thermal performance curves using rTPC**. (1) AIC, BIC or other metrics of model fit can be calculated to help to allow model selection or model averaging. (2) If TPCs are being fit to averages of multiple replicates, then the pipeline can incorporate model weights (1/standard deviation) that can change the model fit and the value of estimated parameters. (3) After the model has been fitted, bootstrapping approaches can help visualise and calculate model uncertainty.
