...
To show each function reported previously in the literature, we shall fit the models using the `R` package `nls.multstart`. First, we shall install other packages that will help in model fitting and plotting.
...
We can now load in the data and the packages to fit the TPCs. Here, we shall use some TPCs of photosynthesis and respiration from Padfield _et al._ (2016).

```{r quicklook, out.width = '50%', fig.height = 4, fig.width = 6, fig.align='center', message=FALSE, warning=FALSE}
# load in packages
library(purrr)
library(dplyr)
library(tidyr)
...
Using tools from the `tidyverse`, we can now easily estimate predictions of each model and plot them. In addition, we can estimate extra parameters for each model using `est_params()`.
...
From these fits, we can also take advantage of model selection methods such as the _AICc_ score. We can calculate model weights of these to see which model is most supported. 

```{r model_selection, fig.height=5, fig.width = 6, out.width = '50%'}
...
We can then calculate model weighted predictions of best overall model fit easily enough if desired.

```{r model_average, fig.height=5, fig.width = 6, out.width = '50%'}
...
The amazing thing about this method is that it can easily be implement on many different curves. For example we can fit this model ensemble to each of 15 curves from the dataset, calculate the *AICc* for each one and then plot the model averaged curve.

```{r scale_up, warning=FALSE, cache=TRUE}
# filter 15 curves
...
We can then make the exact plots as we made previously. Firstly of the predictions of each curve.

```{r plot_preds_many, fig.height = 10, fig.width = 10}
...
We can also plot the distribution of the weights across all of the curves.

```{r plot_weights_many, fig.height = 6, fig.width = 8, out.width = '60%'}
