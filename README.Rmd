...
```
From these fits, we can also take advantage of model selection methods such as the _AICc_ score. We can then take model weights of these to see which model is most supported. 
...
We can calculate model weighted predictions of best overall model fit easily enough if desired.
...
We can then easily implement this code on many different models and plot the output. For example we can fit this model ensemble to each of 15 curves from the dataset, calculate the **AICc** for each one and then plot the model averaged curve.
