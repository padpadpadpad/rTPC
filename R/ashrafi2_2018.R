...
#' @details Equation:
#' \deqn{rate=a + b \cdot temp^{\frac{3}{2}} + c \cdot temp^{2}}{%
#' rate = a + b.temp^(3/2) + c.temp^2}
#'
...
#' theme_bw()
#' @export ashrafi2_2018

ashrafi2_2018 <- function(temp, a, b, c){
  est <- a + b*temp^(3/2) + c*temp^2
  return(est)
}

ashrafi2_2018.starting_vals <- function(d){
...
ashrafi2_2018.lower_lims <- function(d){
  a = -Inf
  b = -Inf
  c = -Inf
...
ashrafi2_2018.upper_lims <- function(d){
  a = Inf
  b = Inf
  c = Inf
