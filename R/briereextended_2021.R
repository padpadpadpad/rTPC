...
#' @details Equation:
#' \deqn{rate=a \cdot temp \cdot (temp - t_{min})^b \cdot (t_{max} - temp)^d}{%
#' rate = a.temp.(temp - tmin)^b.(tmax - temp)^d}
#'
...
#' theme_bw()
#' @export briereextended_2021

briereextended_2021 <- function(temp, tmin, tmax, a, b, d){
  est <- a*temp * (temp - tmin)^b * (tmax - temp)^(d)
  return(est)
}

briereextended_2021.starting_vals <- function(d){
...
briereextended_2021.lower_lims <- function(d){
  tmin = -50
...
briereextended_2021.upper_lims <- function(d){
