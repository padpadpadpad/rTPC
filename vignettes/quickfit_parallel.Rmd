...
Excitingly, [**purrr**]https://www.tidyverse.org/blog/2025/07/purrr-1-1-0-parallel/) now supports parallelisation when combined with [**mirai**](https://mirai.r-lib.org/). This is the reason why we recommend using a single **map()** call that encompasses all models, as it integrates much better with **mirai** and allows for parallelisation.
...
We will fit 5 models to all 60 TPCs in `chlorella_tpc`. Firstly I will demonstrate this using **quickfit_multi_tpcs()** and **purrr**. As curve fitting is generally the most time-consuming part of the pipeline, we fit TPCs with and without parallelisation to see how much **mirai** speeds things up. To switch things up, the models we will fit are **briereextended_2021**, **gaussianmodified_2006**, **briere1simplified_1999**, **tomlinsonphillips_2015**, and **lactin2_1995**.
...
```{r fit_many_parallel, warning=FALSE}
# start timer
  ...
  mutate(
    mods = map(
      ...
      in_parallel(
        ...
        quickfit_tpc_multi = quickfit_tpc_multi
      ),
      .progress = TRUE
    )
  ) %>%
...
This code took `r as.numeric(custom_parallel_end - custom_parallel_start, units = 'secs')` seconds to run. We can then plot all the curves as done in previous vignettes.
