...
d_boots <- mutate(d_boots, refit = map(strap, ~ nls_multstart_progress(rate~sharpeschoolhigh_1981(temp = temp, r_tref,e,eh,th, tref = 15),
                        ...
                        supp_errors = 'Y',
                        convergence_count = FALSE)),
                  # predict over new temperature data
                  preds =  map(refit, ~augment(.x, newdata = new_temp_data)))
```
...
# create new temperature data
new_temp_data2 <- tibble(temp = seq(min(d2$temp), max(d2$temp), by = 0.1))
...
d_boots <- mutate(d_boots, preds = map(refit, ~augment(.x, newdata = new_temp_data2)))
